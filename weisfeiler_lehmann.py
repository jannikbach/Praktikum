...
# invariant
def wl_init(graph) -> nx.Graph:
    return weisfeiler_lehmann_init(graph, node_attr=["element", "charge"], edge_attr=["order"])


# invariant
def wl_step(graph) -> nx.Graph:
    return weisfeiler_lehmann_step(graph)


def weisfeiler_lehmann_init(graph: nx.Graph, node_attr: list[str], edge_attr: list[str]) -> nx.Graph:
    """
...
def weisfeiler_lehmann_step(graph: nx.Graph) -> nx.Graph:
    new_hashes = {}  # store hashes for the next iteration (don't override the current ones)
...
def _node_label(node, node_attrs):
    return ",".join([str(node[a]) for a in node_attrs])


def _edge_label(edge, edge_attrs):
    return ",".join([str(edge[a]) for a in edge_attrs])
